...
export class UserNeoDBService {
    ...
    static async saveCurrentSnapshot(path: string): Promise<void> {
        try {
            // Get current snapshot from the store
            const store = localStoreService.getStore();
            const serialized = store.serialize();
            
            // Create a snapshot with the serialized data
            const snapshot = {
                document: {
                    store: serialized,
            ...
            // Save the snapshot
            await this.saveNodeSnapshot(path, snapshot);
            logger.debug('neo4j-service', '💾 Saved current snapshot', { path });
        } catch (error) {
            logger.error('neo4j-service', '❌ Failed to save current snapshot:', error);
            throw error;
        }
    }

    static async clearCanvasAndLoadSnapshot(
        path: string, 
        setLoadingState: (state: LoadingState) => void
    ): Promise<void> {
        try {
            // Clear the canvas first
            localStoreService.getStore().clear();
            logger.debug('neo4j-service', '🧹 Cleared canvas');

            // Then load the new snapshot
            const snapshot = await this.loadNodeSnapshot(path);
            if (snapshot) {
                await localStoreService.loadSnapshot(snapshot, setLoadingState, false);
                logger.info('neo4j-service', '✅ Loaded snapshot into store', { path });
            } else {
                setLoadingState({ status: 'error', error: 'No snapshot found' });
            }
        } catch (error) {
            logger.error('neo4j-service', '❌ Failed to clear canvas and load snapshot:', error);
            setLoadingState({ 
                status: 'error', 
                error: error instanceof Error ? error.message : 'Failed to load snapshot' 
            });
